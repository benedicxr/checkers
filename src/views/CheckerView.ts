...
export class CheckerView {
  ...
  #onCellClick: CellClickHandler | null = null;
  ...
  constructor(
    ...
    if (!boardEl) throw new Error(`Board element not found: #${boardElementId}`);
  ...
  bindUndoClick(handler: () => void): void {
  ...
  #attachListeners(): void {
    this.#boardElement.addEventListener("click", (event: MouseEvent) => {
      ...
      const cell = event.target.closest<HTMLElement>(`.${CSS_CLASSES.CELL}`);
  ...
  #attachDndListeners(): void {
    this.#boardElement.addEventListener("dragstart", (event: DragEvent) => {
      if (!this.#onCellClick) return;
      if (!(event.target instanceof Element)) return;

      const piece = event.target.closest<HTMLElement>(`.${CSS_CLASSES.PIECE}`);
      if (!piece || !this.#boardElement.contains(piece)) return;

      const cell = piece.closest<HTMLElement>(`.${CSS_CLASSES.CELL}`);
      if (!cell) return;
    ...
    this.#boardElement.addEventListener("dragleave", (event: DragEvent) => {
      if (!(event.target instanceof Element)) return;
      const cell = event.target.closest<HTMLElement>(`.${CSS_CLASSES.CELL}`);
  ...
  #setDropHover(cellEl: HTMLElement | null): void {
  ...
  render(boardModel: BoardSnapshot, { animate = true }: { animate?: boolean } = {}): void {
    ...
    boardModel.forEach((row, rowIndex) => {
      row.forEach((checker, colIndex) => {
        const cell = this.#boardElement.querySelector<HTMLDivElement>(`[data-row="${rowIndex}"][data-col="${colIndex}"]`);
        if (!cell) return;
    ...
    const newPieces = [...this.#boardElement.querySelectorAll<HTMLElement>(`.${CSS_CLASSES.PIECE}[data-id]`)];
  ...
  #createPiece(checker: CheckerSnapshot): HTMLDivElement {
    ...
    piece.dataset.id = String(checker.id);
    piece.draggable = true;
    return piece;
  }

  highlightCell(row: number | null, col: number | null): void {
  ...
  highlightMoves(moves: Move[]): void {
    this.#boardElement
      .querySelectorAll(`.${CSS_CLASSES.AVAILABLE_STEP}`)
      .forEach((el) =>
        el.classList.remove(CSS_CLASSES.AVAILABLE_STEP, CSS_CLASSES.AVAILABLE_MOVE, CSS_CLASSES.AVAILABLE_CAPTURE),
      );
  ...
  highlightCapturablePieces(coordsList: Coords[]): void {
    this.#boardElement
      .querySelectorAll(`.${CSS_CLASSES.PIECE}.${CSS_CLASSES.CAPTURABLE}`)
